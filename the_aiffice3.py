...
task1 = Task(description='''You are the CEO of a dev team called The AIffice. 
             On your team you have a programmer, a designer, and a user/tester.
             
             break down the following project into steps:  
Build a website for a company called "The AIffice". 
The AIffice produces software for clients with as little hassle as possible.
Build and iterate upon separate, but connected HTML pages, CSS stylesheet, and JavaScript files
for your company's (The AIffice) website write. 
Build a full web UI for a respensive web app. 
The navbar and pages are split into "The Team!", "Projects", and "Episodes", "Hire Us", "Help Us".
Divide the page nicely using divs and containers. Use the colors matte orange and light matte grey. 
The client wants you to think about this job step-by-step before you output the code.
''',agent=researcher)
task2 = Task(description='''Give exact instructions to your teammate, based on the project handed to you.

''',agent=designer)
task3 = Task(description='''Think about the project that's been proposed. Build out the entire code.''', agent=coder)
task4 = Task(description='''You've been handed code for a website. Return the code and include dall-e generated images of "An old fashinoed comic book style, depicting a team outside their company's (called The AIffice) building.''', agent=designer)
task5 = Task(description='''Review the code, and make sure the images generate. If either doesn't work, please hand it back to the appropriate teammate to resolve.''', agent=researcher)
task6 = Task(description='''Return the code to the programmer. Including your design instructions as commented out // for the developer to implement. ''', agent=designer)
task7 = Task(description='''Review every step from the designer, and improve your previous code files accordingly. 
             Output each new file one at a time. Use Bootstrap CSS to bring the website to an enterprise level. ''', agent=coder)
